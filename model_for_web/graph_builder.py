...
class GraphBuilder:
	...
	def build_homogeneous_graph(self, use_features: bool = True) -> Data:
		...
		# Node features: if use_features is True, expect preprocessed arrays; otherwise
		# create a minimal zero feature vector per node so downstream code that
		# accesses graph.x still works.
		if use_features:
			...
			# create a single-dimension zero feature for each node
			student_features = torch.zeros((self.num_students, 1), dtype=torch.float32)
			course_features = torch.zeros((self.num_courses, 1), dtype=torch.float32)
