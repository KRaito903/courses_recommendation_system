...
class DataPreprocessor:
    ...
    def preprocess_data(data: Dict, max_semester: int = 12, gpa_scale: float = 10.0, num_majors: int = 9,
        ...
        # Ensure student and course feature arrays are homogeneous before stacking
        student_features_arr = np.array(preprocessed_students, dtype=np.float32)

        # Preprocessed courses may still vary in length (if external data had differing weight lengths).
        # Convert each to a 1-D numpy array and pad to the maximum length.
        course_arrays = [np.asarray(x, dtype=np.float32).ravel() for x in preprocessed_courses]
        max_course_len = max([a.size for a in course_arrays]) if course_arrays else 0
        padded_courses = [np.pad(a, (0, max_course_len - a.size), mode='constant', constant_values=0.0) for a in course_arrays]
        course_features_arr = np.vstack(padded_courses).astype(np.float32) if padded_courses else np.zeros((0, 0), dtype=np.float32)

        preprocessed_data = {
            'students': data['students'],
            'courses': data['courses'],
            'enrollments': preprocessed_enrollments,
            'student_features': student_features_arr,
            'course_features':  course_features_arr
        }
        if is_save_json:
